...
SELECT
    ...
    salary_hour_avg,
    CASE
        WHEN salary_hour_avg < 23 THEN 'Low'
        -- WHEN salary_hour_avg BETWEEN 25 AND 50 THEN 'Medium'
...
-- Investigate the `job_title` column values
SELECT
    job_title,
    job_title_short
FROM job_postings_fact
...
-- Randomly select 10 rows from the `job_postings_fact` table
SELECT
    job_title,
    job_title_short
FROM job_postings_fact
...
-- Calculate Median Salariees for Different Buckets
    -- < $100K
    -- >= $100K
...
-- Compute a standardized_salary using yearly salary and adjusted hourly salary (e.g. 2080 hrs/year)
